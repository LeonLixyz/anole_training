...
if __name__ == '__main__':
    ...
    print(f"args.custom_dataset_path: {args.custom_dataset_path}")
    ...
    # Merge all splits into one dataset for evaluation
    from datasets import concatenate_datasets
    
    splits_to_merge = []
    for split_name in ['train', 'validation', 'eval', 'dev', 'test']:
        if split_name in data:
            splits_to_merge.append(data[split_name])
            print(f"Adding {split_name} split with {len(data[split_name])} examples")
    
    if not splits_to_merge:
        raise ValueError("No suitable split found in dataset for evaluation.")
    
    eval_split = concatenate_datasets(splits_to_merge)
    print(f"Created merged evaluation dataset with {len(eval_split)} examples")
    
    # Use the merged dataset for all splits
    ...
    if args.model in ['anole']:
        ...
        kwargs['stopping_criteria'] = StoppingCriteriaList([StopStringCriteria(stop_strings=["<reserved08706>", ""], tokenizer=processor.tokenizer)])
        # used in evaluation during training
        training_args.customize_gen_stopping_criteria = StoppingCriteriaList([StopStringCriteria(stop_strings=["<reserved08706>", ""], tokenizer=processor.tokenizer)])
